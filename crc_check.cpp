...
{
	uint8_t CRC8 = 0;
	if ((message == NULL) || (length <= 2)) return;
	CRC8 = Get_CRC8_Check_Sum( (uint8_t *)message, length-1, CRC8_INIT);
	message[length-1] = CRC8;
}
...
void Append_CRC16_Check_Sum(uint8_t * message,uint32_t length)
{
	...
	CRC16 = Get_CRC16_Check_Sum( (uint8_t *)message, length-2, CRC16_INIT );
	message[length-2] = (uint8_t)(CRC16 & 0x00ff);
	message[length-1] = (uint8_t)((CRC16 >> 8)& 0x00ff);
}
